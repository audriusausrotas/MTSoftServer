...
export function transformFencesForProduction(
  ...
  return project.fenceMeasures.reduce((acc: any[], item: any) => {
    ...
    if (!currentFence) return acc;
    if (
      currentFence.category !== "Tvora" &&
      currentFence.category !== "Žaliuzė"
    )
      return acc;
...
export async function updateMeasure(data: any, user: User) {
  ...
  if (
    (field === "cut" || field === "done" || field === "holes") &&
    quantity > 0
  ) {
    const event = buildProductionEvent(
      data,
      quantity,
      savedProject,
      user,
...
function buildProductionEvent(
  ...
  const isBinding =
    data.measureIndex === undefined || data.measureIndex === null;
      ...
      length:
        data.field === "holes"
          ...
          : isBinding
            ...
            : project.fences?.[data.index]?.measures?.[data.measureIndex]
                ?.length,
